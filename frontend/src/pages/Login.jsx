...
const Login = () => {
  ...
  const {token, setToken, navigate, baseApiUrl, getUserCart} = useContext(ShopContext);
  const [currentState, setCurrentState] = useState('Login');
  const [name, setName] = useState('Tharusan');
  const [email, setEmail] = useState('tharusan@forever.com');
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if(currentState === 'Sign Up') {
        response = await axios.post(baseApiUrl + '/api/user/register', {name, email, password});
      } else {
        response = await axios.post(baseApiUrl + '/api/user/login', {email, password});
      }
      
      if(response.data.success) {
        var bearerToken = 'Bearer ' + response.data.result.token;
